...
describe('As PRODUCER ', () => {
  it('I should not be able to create a new MV.', async () => {
    ...
    const invokeResponse: ChaincodeResponse = await new MVMockTransaction(
      mockStub
    ).createMV(mv, producerOrganization, isErrorExpected);
  ...
  it('I should be able to update a comptage MV when I have permission.', async () => {
    ...
    const invokeResponse: ChaincodeResponse = await new MVMockTransaction(
      mockStub
    ).updateMV(mv, producerOrganization);
    ...
    expect(updatedMV.resolution).equal(30005);
  });

  it('I should not be able to update a comptage MV when I do not have permission.', async () => {
    mockStub.setCreator(tsoOrganizationType.mspId);
    ...
    await new MVMockTransaction(mockStub).createMV(mv2, dsoOrganization);

    const isErrorExpected = true;
    mockStub.setCreator(producerOrganizationType.mspId);
    mv2.resolution = 30010;
    const invokeResponse: ChaincodeResponse = await new MVMockTransaction(
      mockStub
    ).updateMV(mv2, producerOrganization, isErrorExpected);
  ...
  it('I should be able to get a comptage MV.', async () => {
    mockStub.setCreator(tsoOrganizationType.mspId);
    await new EDAMockTransaction(mockStub).createEDA(eda, tsoOrganization);
    ...
    const queryResponse: QueryResponse<MV> = await new MVMockTransaction(
      mockStub
    ).getMVById(mv.energyAccountMarketDocumentMrid, producerOrganization);
